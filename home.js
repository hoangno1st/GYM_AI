document.addEventListener('DOMContentLoaded', function() {
    ...
    // Biến theo dõi các ngày đã điểm danh
    let checkedInDates = JSON.parse(localStorage.getItem('checkedInDates')) || [];

    // Các biến cho calendar
    let currentDate = new Date();
    let currentMonth = currentDate.getMonth();
    let currentYear = currentDate.getFullYear();

    // Hàm kiểm tra xem có thể điểm danh cho ngày được chọn hay không
    function canCheckInForDate(dateStr) {
        // Chỉ cho phép điểm danh cho ngày trong quá khứ hoặc ngày hiện tại
    ...
    // Hàm điểm danh cho một ngày cụ thể
    function checkInForDate(dateStr) {
        if (!canCheckInForDate(dateStr)) {
            Swal.fire({
                icon: 'error',
                title: 'Không thể điểm danh',
                text: 'Bạn không thể điểm danh cho ngày trong tương lai',
        ...
        if (!checkedInDates.includes(dateStr)) {
            Swal.fire({
                ...
                text: `Bạn muốn điểm danh cho ngày ${new Date(dateStr).toLocaleDateString('vi-VN')}?`,
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    ...
                    // Cập nhật thống kê nếu là ngày hôm nay
                    const today = new Date().toISOString().split('T')[0];
                    if (dateStr === today) {
                        // Gọi hàm updateWorkoutStats từ stats.js
                        const workoutData = {
                    ...
                    Swal.fire({
                        ...
                        text: 'Bạn đã điểm danh thành công cho ngày đã chọn',
                        showConfirmButton: false,
                        timer: 1500
                    });
            ...
            Swal.fire({
                icon: 'info',
                title: 'Đã điểm danh',
                text: 'Bạn đã điểm danh cho ngày này rồi',
                showConfirmButton: false,
                timer: 1500
            });
        }
    ...
    function generateCalendar(year, month) {
        ...
        if (!calendarBody) return; // Đảm bảo element tồn tại
        ...
        for (let i = 0; i < 6; i++) {
            ...
            for (let j = 0; j < 7; j++) {
                ...
                if (i === 0 && j < startingDay) {
                    ...
                    // Thêm sự kiện click để điểm danh
                    cell.addEventListener('click', function() {
                        const dateStr = this.dataset.date;
                        if (dateStr) {
                            checkInForDate(dateStr);
                        }
                    });
    ...
    // Khởi tạo lịch khi trang tải xong
    document.addEventListener('DOMContentLoaded', function() {
        // Khởi tạo lịch
        updateCalendarTitle();
        generateCalendar(currentYear, currentMonth);
        ...
        // Xử lý nút điểm danh
        const checkinBtn = document.getElementById('checkin-workout');
        if (checkinBtn) {
            checkinBtn.addEventListener('click', function() {
                const today = new Date();
                const dateString = today.toISOString().split('T')[0];
                checkInForDate(dateString);
            });
        }
